...
%     mlb.bayesType = 3; % Comment In to decode using Gaussian rather than Poisson
    ...
    fiscLOO_Posts(ani) = {mean(cell2mat(mlb.post),3,'omitnan')};
    % Decode Time
    timeDecode = cell2mat(mlb.DecodeBayesPost(mlb.post, mlb.decodeIDvects(:,1)));
    timePoints = unique(mlb.decodeIDvects(:,1));
    ...
    % Decode Position
    posDecode = cell2mat(mlb.DecodeBayesPost(mlb.post, mlb.decodeIDvects(:,3)));
    positions = unique(mlb.decodeIDvects(:,3));
    ...
    % Decode Odor
    odrDecode = cell2mat(mlb.DecodeBayesPost(mlb.post, mlb.decodeIDvects(:,4)));
    odors = unique(mlb.decodeIDvects(:,4));
...
%     
    ...
    timePosts = nan(size(mlb.post,1), size(mlb.post,2), length(trlPoss));
    for pos = 1:length(trlPoss)
        timePosts(:,:,pos) = mean(mlb.post(:,:,trlPosVect==trlPoss(pos) & trlPerfVect==1),3, 'omitnan');
        for t = 1:length(timePoints)
            timeAccuracy(:,t,pos) = mean(timeDecode(:,trlPosVect==trlPoss(pos) & trlPerfVect==1)==timePoints(t),2, 'omitnan');
        end
    end
    ...
    for pos = 1:length(positions)
        for p = 1:length(positions)
            posAccuracy(:,p,pos) = mean(posDecode(:,trlPosVect==positions(pos) & trlPerfVect==1)==positions(p),2, 'omitnan');
        end
    end
    fiscISC_Decodes{2,1,ani} = posAccuracy; 
    % Decode Odor
    odrDecode = mlb.DecodeBayesPost(mlb.post, mlb.decodeIDvects(:,4));
    odors = unique(mlb.decodeIDvects(:,4));
    odrAccuracy = nan(size(odrDecode,1),length(odors), length(odors));
    for odr = 1:length(odors)
        for p = 1:length(odors)
            odrAccuracy(:,p,odr) = mean(odrDecode(:,trlOdrVect==odors(odr) & trlPerfVect==1)==odors(p),2, 'omitnan');
        end
    end
...
annotation(gcf,'textbox', [0.1 0.95 0.9 0.05],...
    'String', sprintf("FISC Leave-One-Out Group: binSize = %.0fms, dsRate = %.0fms, Trial Window = (%.0fms:%.0fms to %s), BayesType = %.0f",...
...
for p = 1:size(fiscISC_Posts{1},3)
    ...
    plot(get(gca, 'xlim'), repmat(find(mlb.obsvTimeVect==nearestPOtime)+0.5, [1,2]), '--k', 'linewidth', 2);
    plot(get(gca, 'xlim'), repmat(find(mlb.obsvTimeVect==nearestRWDtime)+0.5, [1,2]), ':k', 'linewidth', 2);
...
annotation(gcf,'textbox', [0.05 0.95 0.8 0.05],...
    'String', sprintf("FISC Decode ISC Group: binSize = %.0fms, dsRate = %.0fms, Trial Window = (%.0fms:%.0fms to %s), BayesType = %.0f",...
    binSize, dsRate, trlWindow{1}(1), trlWindow{1}(2), alignment{1}, bayesType),...
    'FontSize',10, 'edgecolor', 'none', 'horizontalalignment', 'left');
