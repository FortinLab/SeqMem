...
groupPostOvP_TAO = cell(mlb.seqLength^2,3);         chancePostOvP_TAO = cell(1,3,2);
% Decodings
...
groupDecodeOvP_TAO = cell(1,3);                     chanceDecodeOvP_TAO = cell(1,3,2);
...
for odr = 1:size(realPost_TransMat,1)
    for pos = 1:size(realPost_TransMat,2)
        ...
        if odr~=pos
            ...
            %% OutSeq
            % Posteriors
            if ~isempty(groupPostPos_TransMat{odr,pos})
                groupPostOvP_OutSeq{osNdx} = [groupPostPos_TransMat{odr,pos}(:,pos,:), groupPostPos_TransMat{odr,pos}(:,odr,:)];
            end
            if ~isempty(groupPostPos_TAO{odr,pos})
                groupPostOvP_TAO{osNdx} = [groupPostPos_TAO{odr,pos}(:,pos,:), groupPostPos_TAO{odr,pos}(:,pos,:), nan(length(mlb.obsvTimeVect),1, size(groupPostPos_TAO{odr,pos},3))];
                if pos~=4
...
for perm = 1:numChancePerms
    tempOS = cell(1,2,mlb.seqLength^2);
...
chancePostOvP_OutSeq{1,2,2} = std(tempPostOvP_OS(:,1,:),0,3);
...
save('PFC_OsTAOs_Decodings.mat', '-v7.3');
